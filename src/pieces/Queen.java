...
public final class Queen extends Piece {
      ...
      public Queen(int color, Position startingPos) {
      ...
      public void loadSprite(boolean isWhite) {
            if(isWhite){
                  try { super.scaleAndSetSprite(ImageIO.read( getClass().getResource(Settings.DEFAULT_WHITE_IMG_PATH + "/queen.png") ) ); }
                  catch(IOException e) {System.out.println("Error loading sprite.\n" + e);}
            } else {
                  try { super.scaleAndSetSprite( ImageIO.read( getClass().getResource(Settings.DEFAULT_BLACK_IMG_PATH + "/queen.png") ) ); }
                  catch(IOException e) {System.out.println("Error loading sprite.\n" + e);}
            }
      }
      ...
      public void calcLegalMoves(Board board) {
            ...
            Position[][] directions = {   { new Position(0,1), new Position(1,0), new Position(0,-1), new Position(-1,0)},          // Rook moves
                                                      { new Position(1,1), new Position(1,-1), new Position(-1,1), new Position(-1,-1)} };     // Bishop moves
            legalMoves.clear();
            
            for(Position[] directionsArr : directions)
                  for(Position dir : directionsArr){
                        int offset = 1;
