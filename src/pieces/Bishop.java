...
public final class Bishop extends Piece {
      ...
      public Bishop(int color, Position startingPos) {
      ...
      public void loadSprite(boolean isWhite) {
            if(isWhite){
                  try { super.scaleAndSetSprite(ImageIO.read( getClass().getResource(Settings.DEFAULT_WHITE_IMG_PATH + "/bishop.png") ) ); }
                  catch(IOException e) {System.out.println("Error loading sprite.\n" + e);}
            } else {
                  try { super.scaleAndSetSprite( ImageIO.read( getClass().getResource(Settings.DEFAULT_BLACK_IMG_PATH + "/bishop.png") ) ); }
                  catch(IOException e) {System.out.println("Error loading sprite.\n" + e);}
            }
      }
      ...
      public void calcLegalMoves(Board board) {
            ...
            Position[] directions = { new Position(1,1),
                                                  new Position(1,-1),
                                                  new Position(-1,1),
                                                  new Position(-1,-1)};
            legalMoves.clear();
